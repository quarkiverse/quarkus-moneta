    ...
    private RateResult findExchangeRate(ConversionQuery conversionQuery) {
        ...
        if (dates == null) {
            ...
            LocalDate date = this.rates.keySet().stream()
                    .max(comparator)
                    .orElseThrow(
                            () -> new MonetaryException("There is not more recent exchange rate to  rate on ECBRateProvider."));
            ...
            String datesOnErros = Stream.of(dates).map(date -> date.format(DateTimeFormatter.ISO_LOCAL_DATE))
                    .collect(Collectors.joining(","));
            throw new MonetaryException(
                    "There is no exchange on day " + datesOnErros + " to rate to  rate on ECBRateProvider.");
        }
