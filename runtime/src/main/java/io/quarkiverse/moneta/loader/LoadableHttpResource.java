    ...
    protected boolean load(URI itemToLoad, boolean fallbackLoad) {
        ...
        try {
            if (fallbackLoad) {
                ...
                var proxyPort = this.properties.get("proxy.port");
                var proxyHost = this.properties.get("proxy.host");

                if (proxyPort != null && proxyHost != null) {
                    var resolver = ProxySelector.of(InetSocketAddress.createUnresolved(proxyHost, Integer.parseInt(proxyPort)));
                    builder.proxy(resolver);
                }

                var connectTimeout = this.properties.get("connection.connect.timeout");
                if (connectTimeout != null) {
                    int seconds = Integer.parseInt(connectTimeout);
                    builder.connectTimeout(Duration.ofSeconds(seconds));
                }
                ...
                if (Arc.container() != null) {
                    ...
                    if (registry != null) {
                        registry.get(Constants.TLS_CONFIGURATION)
                                .ifPresent(configuration -> {
                                    try {
                                        var sslContext = configuration.createSSLContext();
                                        builder.sslContext(sslContext);

                                        LOG.fine("Set SSL Context from TLS registry.");
                                    } catch (Exception e) {
                                        LOG.log(Level.SEVERE, "Failed to create SSLContext", e);
                                    }
                                });
                    }
                }
