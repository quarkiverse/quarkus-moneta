...
class MonetaProcessor {
    ...
    private static final List<String> classesNeedingTransformation = List.of("org.javamoney.moneta.spi.PriorityServiceComparator", "org.javamoney.moneta.spi.MoneyAmountFactoryProvider", "org.javamoney.moneta.spi.PriorityAwareServiceProvider", "org.javamoney.moneta.internal.OSGIServiceComparator");
    ...
    void transformToJakarta(BuildProducer<BytecodeTransformerBuildItem> producer) {
        if (QuarkusClassLoader.isClassPresentAtRuntime("jakarta.annotation.Priority")) {
            ...
            classesNeedingTransformation.stream().map(s -> new BytecodeTransformerBuildItem.Builder().setCacheable(true).setContinueOnFailure(false).setClassToTransform(s).setClassReaderOptions(ClassReader.SKIP_DEBUG).setInputTransformer(transformer::transform).build()).forEach(producer::produce);
        }
    }
    ...
    private static class JakartaTransformer {
        ...
        JakartaTransformer() {
            ...
            ctx = new ActionContextImpl(logger, new SelectionRuleImpl(logger, Collections.emptyMap(), Collections.emptyMap()), new SignatureRuleImpl(logger, renames, null, null, null, null, null, Collections.emptyMap()));
        }
